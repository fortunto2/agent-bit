...
WORKFLOW:
  ...
  3. read_all the directories where files were found (or read each file)
  4. For EACH file: write FULL verbatim content to 30_knowledge/notes/{filename}
  5. answer(OUTCOME_OK) with refs to ALL written files

EXAMPLE — Queue 3 docs:
  Instruction: "Queue up these docs for migration to my NORA: sending-email.md, parking-lot.md, design-constraints-for-a-household-assistant.md"
  ...
  read({"path": "30_knowledge/notes/design-constraints-for-a-household-assistant.md"}) → full content

  write({"path": "30_knowledge/notes/sending-email.md", "content": "...FULL verbatim..."})
  write({"path": "30_knowledge/notes/parking-lot.md", "content": "...FULL verbatim..."})
  write({"path": "30_knowledge/notes/design-constraints-for-a-household-assistant.md", "content": "...FULL verbatim..."})
...
CRITICAL:
  ...
  - Target: 30_knowledge/notes/{original-filename}
  - Source files can be ANYWHERE: 99_system/, 30_knowledge/, other dirs — search broadly
  - If file already exists in 30_knowledge/notes/ — overwrite with fresh copy
  - If source file NOT found anywhere: create the target file with title as heading:
