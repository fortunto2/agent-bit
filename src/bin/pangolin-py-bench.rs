...
async fn main() -> Result<()> {
    dotenvy::dotenv().ok();
    // Phoenix: separate project "pac1-pangolin-py" to keep traces distinct from main agent.
...
async fn run() -> Result<()> {
    ...
    let session = PangolinSession::with_context(pcm.clone(), ctx_json, &cli.task);
    let tool = ExecutePyTool { session: session.clone() };

    let system_prompt = std::fs::read_to_string("prompts/pangolin-py.md")
