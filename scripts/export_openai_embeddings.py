#!/usr/bin/env python3
"""Pre-compute OpenAI text-embedding-3-small class centroids.

Usage:
    uv run --with openai scripts/export_openai_embeddings.py

Requires: OPENAI_API_KEY environment variable.

Outputs to models/:
    openai_class_embeddings.json — pre-computed class embeddings [(label, [f32; 1536])]
"""
...
# Inline class definitions (avoid importing export_model.py which needs heavy deps)
SECURITY_CLASSES = {
    "injection": [
...
def compute_openai_centroids():
    """Compute class centroids using OpenAI text-embedding-3-small."""
    ...
    print(f"Computing OpenAI embeddings for {len(all_classes)} classes...")

    centroids = []
    for label, examples in all_classes.items():
        # Embed all examples for this class
